...
fun WeatherScreen(
    ...
    val loadingProgress by remember { mutableStateOf(0f) }
    val isLoading by remember { mutableStateOf(false) }
    var dateText by remember { mutableStateOf("2025-01-01") }
    Column {
        // TextField for entering the date
        if (isLoading) {
            LinearProgress(loadingProgress, modifier = Modifier.align(Alignment.CenterHorizontally))
        }
        else {
        ...
        // Display loading progress if isLoading is true
            ...
            LazyColumn {
                item {
                    ElevatedCardExample(name = "Min",MINtemp)
                }
                item {
                    ElevatedCardExample(name = "Max",MAXtemp)
                }
            }
        }
    }
